...
const Admin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        image_url = supabase.storage.from("flower-images").getPublicUrl(path)
          .data.publicUrl;
      ...
      const { error } = await supabase.from("flowers").insert({
        ...
        image_url,
        image_color: form.image_color || null,
        care_watering: form.care_watering || null,
        care_sunlight: form.care_sunlight || null,
        care_soil: form.care_soil || null,
        care_temperature: form.care_temperature || null,
      });
  ...
  const handleStartEdit = (f: (typeof flowers)[0]) => {
    ...
    setEditForm({
      ...
      season: f.season,
      meaning: f.meaning,
      image_color: f.imageColor ?? "",
      care_watering: f.care?.watering ?? "",
      care_sunlight: f.care?.sunlight ?? "",
      care_soil: f.care?.soil ?? "",
      care_temperature: f.care?.temperature ?? "",
    });
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          care_watering: editForm.care_watering || null,
          care_sunlight: editForm.care_sunlight || null,
          care_soil: editForm.care_soil || null,
          care_temperature: editForm.care_temperature || null,
          ...(image_url ? { image_url } : {}),
        })
        .eq("slug", slug);
      if (error) throw error;
  ...
  const parseSeasons = (s: string): string[] =>
    s
      ? s
          .split(",")
          .map((x) => x.trim())
          .filter(Boolean)
      : [];
  const getSeason = (s: string, idx: number): string =>
    parseSeasons(s)[idx] ?? "";
  const setSeasonAt = (s: string, idx: number, value: string): string => {
    const arr = parseSeasons(s);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-10">
        ...
        <form
          ...
          className="bg-card border border-border rounded-xl p-6 mb-10 space-y-4"
        >
          <div className="flex items-center justify-between flex-wrap gap-2">
            <h3 className="font-kanit font-semibold text-lg text-foreground">
              เพิ่มดอกไม้ใหม่
            </h3>
          ...
          {autofilling && (
            <div className="flex items-center gap-2 p-3 bg-primary/10 border border-primary/20 rounded-lg">
              ...
              <span className="font-sarabun text-sm text-primary">
                กำลังดึงข้อมูลจาก Perenual...
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="font-sarabun text-sm text-foreground block mb-1">
                ชื่อไทย *{" "}
                <AutofillBadge text={form.name_th ? "Autofill" : ""} />
              </label>
              <input
                type="text"
                value={form.name_th}
            ...
            <div>
              <label className="font-sarabun text-sm text-foreground block mb-1">
                ฤดูกาล * <AutofillBadge text={form.season ? "Autofill" : ""} />
              </label>
              <select
                value={getSeason(form.season, 0)}
                onChange={(e) =>
                  setForm({
                    ...form,
                    season: setSeasonAt(form.season, 0, e.target.value),
                  })
                }
                className={inputCls}
              >
            ...
            <div>
              ...
              <input
                ...
                className={inputCls}
                placeholder="auto-generated from autofill"
              />
            </div>
            ...
            <div>
              <label className="font-sarabun text-sm text-foreground block mb-1">
                ฤดูกาลที่ 2 (ไม่บังคับ)
              </label>
              <select
                value={getSeason(form.season, 1)}
                onChange={(e) =>
                  setForm({
                    ...form,
                    season: setSeasonAt(form.season, 1, e.target.value),
                  })
                }
                className={inputCls}
              >
                ...
                {seasons
                  .filter((s) => s !== getSeason(form.season, 0))
                  .map((s) => (
                    <option key={s} value={s}>
                      {s}
                    </option>
          ...
          <div>
            <label className="font-sarabun text-sm text-foreground block mb-1">
              คำอธิบาย *{" "}
              <AutofillBadge text={form.description ? "Autofill" : ""} />
            </label>
            <textarea
          ...
          <div>
            ...
            <input
              ...
              className={inputCls}
              placeholder="e.g. ความรัก, ความหวัง"
            />
          </div>
          ...
          <div>
            <label className="font-sarabun text-sm text-foreground block mb-2">
              สีของดอกไม้{" "}
              <AutofillBadge text={form.colors.length > 0 ? "Autofill" : ""} />
            </label>
            <ColorPicker
              colors={form.colors}
              onToggle={(c) => toggleColor(c, setForm)}
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, image_color: e.target.value })
              }
              className={inputCls}
            >
          ...
          <div>
            <label className="font-sarabun text-sm text-foreground block mb-1">
              รูปภาพดอกไม้{" "}
              <AutofillBadge text={autofillImageUrl ? "Autofill" : ""} />
            </label>
            {autofillImageUrl && !imageFile && (
              <div className="mb-2 flex items-center gap-3">
                <img
                  src={autofillImageUrl}
                  alt="autofill preview"
                ...
                <div>
                  ...
                  <button
                    type="button"
                    onClick={() => setAutofillImageUrl("")}
                    className="font-sarabun text-xs text-destructive hover:underline mt-1"
                  >
                    ลบรูป Autofill
                  </button>
                </div>
            ...
            <label className="flex items-center gap-2 px-4 py-2 rounded-lg border border-dashed border-border bg-muted cursor-pointer hover:bg-secondary transition-colors w-fit">
              ...
              <span className="font-sarabun text-sm text-muted-foreground">
                {imageFile
                  ? imageFile.name
                  : "อัปโหลดรูปเอง (จะแทนที่รูป Autofill)"}
              </span>
              <input
                type="file"
                accept="image/*"
          ...
          <details className="border border-border rounded-lg p-4">
            <summary className="font-kanit text-sm text-foreground cursor-pointer">
              ...
              <AutofillBadge
                text={
                  form.care_watering || form.care_sunlight ? "Autofill" : ""
                }
              />
            </summary>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              <input
                type="text"
                placeholder="การรดน้ำ"
                value={form.care_watering}
                onChange={(e) =>
                  setForm({ ...form, care_watering: e.target.value })
                }
                className={inputCls}
              />
              <input
                type="text"
                placeholder="แสงแดด"
                value={form.care_sunlight}
                onChange={(e) =>
                  setForm({ ...form, care_sunlight: e.target.value })
                }
                className={inputCls}
              />
              <input
                type="text"
                placeholder="ดิน"
                value={form.care_soil}
                onChange={(e) =>
                  setForm({ ...form, care_soil: e.target.value })
                }
                className={inputCls}
              />
              <input
                ...
                value={form.care_temperature}
                onChange={(e) =>
                  setForm({ ...form, care_temperature: e.target.value })
                }
                className={inputCls}
              />
            </div>
              ...
              >
                <div className="flex items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-kanit text-sm text-foreground truncate">
                      {f.nameEn} ({f.nameTh})
                    </p>
                    <p className="font-sarabun text-xs text-muted-foreground">
                      {f.season} · {f.colors.join(", ")}
                    </p>
                  </div>
                  <button
                    onClick={() =>
                ...
                {editingSlug === f.id && (
                  <div className="border-t border-border p-4 space-y-3 bg-muted/30">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="font-sarabun text-xs text-muted-foreground block mb-1">
                          ชื่ออังกฤษ *
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        ...
                        <select
                          value={getSeason(editForm.season, 0)}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              season: setSeasonAt(
                      ...
                      <div>
                        ...
                        <select
                          value={getSeason(editForm.season, 1)}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              season: setSeasonAt(
                        ...
                        >
                          <option value="">— ไม่มี —</option>
                          {seasons
                            .filter((s) => s !== getSeason(editForm.season, 0))
                            .map((s) => (
                              <option key={s} value={s}>
                                {s}
                              </option>
                    ...
                    <div>
                      <label className="font-sarabun text-xs text-muted-foreground block mb-2">
                        สีของดอกไม้
                      </label>
                      <ColorPicker
                        colors={editForm.colors}
                        onToggle={(c) => toggleColor(c, setEditForm)}
                      />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          value={editForm.image_color}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              image_color: e.target.value,
                            })
                          }
                          className={inputCls}
                        >
                      ...
                      <div>
                        ...
                        <label className="flex items-center gap-2 px-3 py-2 rounded-lg border border-dashed border-border bg-background cursor-pointer hover:bg-muted transition-colors">
                          ...
                          <span className="font-sarabun text-xs text-muted-foreground">
                            {editImageFile
                              ? editImageFile.name
                              : "เลือกรูปใหม่..."}
                          </span>
                    ...
                    <details className="border border-border rounded-lg p-3">
                      <summary className="font-kanit text-xs text-foreground cursor-pointer">
                        🌱 ข้อมูลการดูแล
                      </summary>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                        <input
                          type="text"
                          placeholder="การรดน้ำ"
                          value={editForm.care_watering}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              care_watering: e.target.value,
                            })
                          }
                          className={inputCls}
                        />
                        <input
                          type="text"
                          placeholder="แสงแดด"
                          value={editForm.care_sunlight}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              care_sunlight: e.target.value,
                            })
                          }
                          className={inputCls}
                        />
                        <input
                          type="text"
                          placeholder="ดิน"
                          value={editForm.care_soil}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              care_soil: e.target.value,
                            })
                          }
                          className={inputCls}
                        />
                        <input
                          type="text"
                          placeholder="อุณหภูมิ"
                          value={editForm.care_temperature}
                          onChange={(e) =>
                            setEditForm({
                              ...editForm,
                              care_temperature: e.target.value,
                            })
                          }
                          className={inputCls}
                        />
