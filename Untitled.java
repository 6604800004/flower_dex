...
import { Plus, Trash2, Upload, Loader2, Pencil, X, Check } from "lucide-react";
...
const Admin = () => {
  ...
  const getSeason = (s: string, idx: number): string => parseSeasons(s)[idx] ?? "";
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-4 py-10">
        ...
        <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 mb-10 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.slug}
                ...
                className={inputCls} placeholder="e.g. rose" />
            </div>
            <div>
              <label className="font-sarabun text-sm text-foreground block mb-1">ชื่ออังกฤษ *</label>
              <input type="text" value={form.name_en}
                onChange={(e) => setForm({ ...form, name_en: e.target.value })}
                className={inputCls} placeholder="e.g. Rose" />
            </div>
            <div>
              <label className="font-sarabun text-sm text-foreground block mb-1">ชื่อไทย *</label>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setForm({ ...form, season: setSeasonAt(form.season, 0, e.target.value) })}
                className={inputCls}
              >
            ...
            <div>
              <label className="font-sarabun text-sm text-foreground block mb-1">ฤดูกาลที่ 2 (ไม่บังคับ)</label>
              <select
                value={getSeason(form.season, 1)}
                onChange={(e) => setForm({ ...form, season: setSeasonAt(form.season, 1, e.target.value) })}
                className={inputCls}
              >
                <option value="">— ไม่มี —</option>
                {seasons.filter((s) => s !== getSeason(form.season, 0)).map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
          </div>

          <div>
            <label className="font-sarabun text-sm text-foreground block mb-1">คำอธิบาย *</label>
          ...
          <div>
            <label className="font-sarabun text-sm text-foreground block mb-2">สีของดอกไม้</label>
          ...
          <div>
            <label className="font-sarabun text-sm text-foreground block mb-1">รูปภาพดอกไม้</label>
            <label className="flex items-center gap-2 px-4 py-2 rounded-lg border border-dashed border-border bg-muted cursor-pointer hover:bg-secondary transition-colors w-fit">
              ...
              <span className="font-sarabun text-sm text-muted-foreground">
                {imageFile ? imageFile.name : "อัปโหลดรูปภาพ"}
              </span>
          ...
          <details className="border border-border rounded-lg p-4">
            <summary className="font-kanit text-sm text-foreground cursor-pointer">🌱 ข้อมูลการดูแล (ไม่บังคับ)</summary>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setEditForm({ ...editForm, season: setSeasonAt(editForm.season, 0, e.target.value) })}
                          className={inputCls}
                        >
                          <option value="">เลือกฤดูกาล</option>
                          {seasons.map((s) => <option key={s} value={s}>{s}</option>)}
                      ...
                      <div>
                        <label className="font-sarabun text-xs text-muted-foreground block mb-1">ฤดูกาลที่ 2 (ไม่บังคับ)</label>
                        <select
                          value={getSeason(editForm.season, 1)}
                          onChange={(e) => setEditForm({ ...editForm, season: setSeasonAt(editForm.season, 1, e.target.value) })}
                          className={inputCls}
                        >
                          <option value="">— ไม่มี —</option>
                          {seasons.filter((s) => s !== getSeason(editForm.season, 0)).map((s) => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </div>
                      <div>
